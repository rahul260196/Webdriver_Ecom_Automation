...
export default class CheckoutsPage extends Page {
   ...
   async userSignin(data) {
       ...
       if (!signinFlag) {
           ...
           try{
               await expect(signin.element.header_myaccount_btn_testid).toBeDisplayed();
           }catch(error){
   ...
   async iftrolleyEmptyAddProduct(type) {
       ...
       console.log("iftrolleyEmptyAddProduct method start");
       let itemValueFlag;
       try {
           console.log("iftrolleyEmptyAddProduct will check for previous items");
           itemValueFlag = await this.isElementDisplayed(Sl.testid(header.element.item_count_testid), this.medium_pause * 2);
           console.log("iftrolleyEmptyAddProduct checking items flag ", itemValueFlag);
       } catch (error) {


       }
       if (!itemValueFlag) {
           console.log("iftrolleyEmptyAddProduct trolley empty so go to open category");
           await this.openCategory();
           console.log("iftrolleyEmptyAddProduct trolley empty navigating to plp");
   ...
   async navigateToPdpFromPlp() {
      ...
      // await browser.saveScreenshot("1.png");
       console.log("navigateToPdpFromPlp method start after medium pause");
       if(!(this.platformName == 'Android')){
       ...
       console.log("navigateToPdpFromPlp brand filter was visible ", brandFlag);
   }
       ...
       console.log("navigateToPdpFromPlp product image was visible ", imageFlag);
       ...
       console.log("navigateToPdpFromPlp plp page title was visible ", pageHeadingFlag);
       ...
       console.log("navigateToPdpFromPlp product count was visible ", productCountFlag);
       const productsLinksElements = await Sl.findElementsByTestIdEndingWith(productlisting.element.product_link_endswithtestid);
       console.log("navigateToPdpFromPlp got the product links ");
       if(this.platformName =='Android'){
   ...
   async addProductToTrolleyFromPdp(type = 'delivery') {
       await browser.pause(this.small_pause);
       console.log("addProductToTrolleyFromPdp start after small pause");
       await helper.waitForDisplayed(Sl.testid(productdetails.element.price_testid), this.long_pause);
       console.log("addProductToTrolleyFromPdp price was visible");
       if (type === 'collection') {
   ...
   async pdpToTrolley() {
       console.log("pdpToTrolley method starts")
       await browser.pause(this.small_pause);
       try {
   ...
   async navigateToTrolley(data, { authentication = 'true', type = 'delivery', firstUseCase = 'false' } = {}) {
       console.log("navigate To Trolley S1");
       console.log("navigate To Trolley Passed authentication ", authentication);
       if (authentication === 'true') {
           console.log("navigate To Trolley User will signin ");
           await this.userSignin(data);
           console.log("navigate To Trolley User will signin end ");
       } else {
           console.log("navigate To Trolley User will logout ");
       ...
       await this.iftrolleyEmptyAddProduct(type);
       console.log("navigate To Trolley After Adding product");
       await this.pdpToTrolley();
