...
export default class SignupPage extends Page {
    ...
    async signUpHeadingWait(data) {
        const signupHeadingElement = await Sl.testid(data.element.signup_heading_testid);
        await helper.waitForDisplayed(signupHeadingElement, this.long_pause);
        await helper.highlightElement(signupHeadingElement);
    }


    async signinButtonFunctionality(data, assert) {
        await this.signUpHeadingWait(data);
        const loginLinkElement = await Sl.testid(data.element.signin_link_testid);
        await helper.waitForDisplayed(loginLinkElement);
        await loginLinkElement.scrollIntoView({ block: 'center', inline: 'center' });
        await helper.highlightElementBeforeClick(loginLinkElement);
        await loginLinkElement.click();
        await asserts.pauseIfHuman(this.small_pause);
        await asserts.pageUrl(assert);
        const signinHeaderElement = await Sl.testid(data.element.signin_heading_text_testid);
        await helper.waitForDisplayed(signinHeaderElement);
        await helper.highlightElement(signinHeaderElement);
        await expect(signinHeaderElement).toBeDisplayed();
