...
export default class Helper {
    ...
    async userSignin(email = signin.value.email, password = signin.value.cred_secure) {
        ...
        if (!isSignedIn) {
            ...
            const signinButton = await Sl.testid(signin.element.signin_btn_testid);
            await this.waitForClickable(signinButton, signin.medium_pause);
            await this.highlightElementBeforeClick(signinButton);
    ...
    async acceptConsent() {
        if (consentflag && (/*signin.base_url.includes(signin.value.preprod_url)  || */
            ...
            await this.waitForDisplayed(consentPopup, signin.long_pause);
    ...
    async userLogout(platformName,username){
        ...
        await expect(Sl.testid(account.element.my_account_heading_testid)).toBeDisplayed();
            ...
            await this.waitForDisplayed(usernameText, signin.long_pause);
            await assert.text(usernameText, username);
        }
        ...
        await assert.pauseIfHuman(this.medium_pause);
            ...
            const signoutButton = await Sl.$('//p[@data-testid="account-about-us-link"]/../../../button');
    ...
    async highlightElementBeforeClick(element, duration = 1000) {
